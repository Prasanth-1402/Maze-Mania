...
import type { Cell, Maze, MazeSettings } from "./types";
...
function unvisitedNeighbors(cells: Cell[], width: number, height: number, cell: Cell): Cell[] {
...
function carveBetween(current: Cell, next: Cell): void {
  if (current.x === next.x) {
    if (current.y > next.y) {
...
function runDfsCarve(cells: Cell[], width: number, height: number, seed: string): void {
...
function resetVisited(cells: Cell[]): void {
  for (const cell of cells) {
    cell.visited = false;
  }
}

function buildMazeOnce(settings: MazeSettings): Maze {
  ...
  const mazeBase: Omit<Maze, "shortestPathLength"> = {
  ...
  const pathLength = shortestPathLength(
    { ...mazeBase, shortestPathLength: -1 },
    { x: 0, y: 0 },
    { x: settings.width - 1, y: settings.height - 1 }
  );

  return { ...mazeBase, shortestPathLength: pathLength };
}

export function generateMaze(settings: MazeSettings): Maze {
  const threshold = pathLengthThreshold(settings.difficulty, settings.width * settings.height);
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const seededSettings: MazeSettings = {
      ...settings,
