...
function getNeighbors(maze: Maze, position: Position): Position[] {
  const { x, y } = position;
...
export function shortestPathLength(
  ...
  const distances = new Array<number>(total).fill(-1);
...
export function canMove(maze: Maze, position: Position, dx: number, dy: number): boolean {
